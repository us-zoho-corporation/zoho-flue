...
- A visualization spec is a sizeable tool-call JSON. Keep the model's output budget high enough (the selected model's `maxTokens`, from its provider `Model` metadata) that the spec **and** the written takeaway fit in one turn — too low a cap truncates the reply mid-stream.
