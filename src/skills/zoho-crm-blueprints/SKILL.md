...
| [Activate Blueprint](references/activate-blueprint.md) | `POST /crm/v8/settings/blueprints/{id}/actions/activate` | Publish/republish a Blueprint draft, or reactivate a deactivated one |
| [Deactivate Blueprint](references/deactivate-blueprint.md) | `POST /crm/v8/settings/blueprints/{id}/actions/deactivate` | Deactivate a currently-active Blueprint |
...
Blueprints, Blueprint States, and Blueprint Transitions each also support `DELETE` at the same path (optionally with `?ids=` for bulk delete) — not covered by its own reference file here, but useful for cleaning up anything created by mistake. Each has its own narrower delete scope alongside the `.ALL` fallback: `ZohoCRM.settings.blueprint.DELETE` (Blueprints), `ZohoCRM.settings.blueprint.states.delete` (states). Zoho's docs list `ZohoCRM.settings.blueprint.transitions.delete` for transitions — untested here, but given the identically-shaped `blueprint.transitions.*` scopes for Get/Create Blueprint Transitions are rejected at the consent screen (see Scopes below), assume this one needs the same `ZohoCRM.settings.transitions.ALL` substitution until verified otherwise.

## Scopes

`ZohoCRM.modules.{module}.READ` / `.UPDATE` (record-level Get/Execute Blueprint Data); `ZohoCRM.settings.blueprint.ALL` (Blueprint/state config) and `ZohoCRM.settings.transitions.ALL` (transition read/create/update/delete) for Blueprint config — both listed separately in `zohoProducts.crm.scopes` (`src/config.ts`). Zoho's own API docs describe the transitions scope as `ZohoCRM.settings.blueprint.transitions.{read,ALL,CREATE,UPDATE,delete}`; that path is wrong and gets rejected at the consent screen with "Invalid OAuth Scope: Scope does not exist" — use `ZohoCRM.settings.transitions.ALL` instead (no `.blueprint.` segment). Neither `ZohoCRM.settings.blueprint.ALL` nor `ZohoCRM.settings.transitions.ALL` come for free with `ZohoCRM.settings.ALL` — Zoho requires each as its own explicitly-granted scope; a user connected before these scopes were added needs to reconnect (or click "Connect all"/"Update" in Settings) to pick them up.
