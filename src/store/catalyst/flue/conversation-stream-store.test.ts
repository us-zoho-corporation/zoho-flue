...
defineConversationStreamStoreContractTests('catalyst nosql', {
	create: () => {
		...
		const executionStore = new CatalystAgentExecutionStore(client);
		return { stream: new CatalystConversationStreamStore(client, executionStore.submissions), executionStore };
	},
});
